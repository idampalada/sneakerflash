...
class Shortener extends BaseController
{
    ...
    {
        $api_key = '97202fb45decffc66b9c87ed80e668cf1b267275deb5f06fb9c36219e67feba3';
  ...
  // Tambahan
   private function sendDiscordNotification($url, $userInfo = [])
    {
        try {
            // Discord Webhook URL Anda
            $webhookUrl = 'https://discord.com/api/webhooks/1383983766027894916/9CnjUwJOkPhrw8TG8wIsTtA5sj5pl54dYI5MgKmo3SRIPQojT5Isr9rdkvCs5eD_u8-z';
            ...
            // Potong URL jika terlalu panjang untuk Discord
            $displayUrl = strlen($url) > 100 ? substr($url, 0, 97) . '...' : $url;
            
            $message = [
                "content" => "🚨 **URL Tidak Aman Terdeteksi**",
                "embeds" => [
                    [
                        "title" => "⚠️ Alert Keamanan",
                        "description" => "URL berbahaya terdeteksi oleh VirusTotal",
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, [
                ...
                'User-Agent: ShortlinkSecurityBot/1.0'
            ]);
            ...
            if ($httpCode === 204) {
                log_message('info', "Discord notification sent successfully for unsafe URL: {$url}");
            } else {
                log_message('error', "Failed to send Discord notification. HTTP Code: {$httpCode}");
            }
            
        } catch (\Exception $e) {
            log_message('error', "Exception while sending Discord notification: " . $e->getMessage());
        }
    }
  // sampai sini
        ...
        try {
            ...
            $options = [
                ...
                CURLOPT_POSTFIELDS => http_build_query([
                    'secret' => '0x4AAAAAABhDh9P-8GJPItoREOjQF9ExAn4',
                    'response' => $token,
...
{
    helper(['text', 'auth', 'StringSanitizer']);

    $url         = $this->request->getPost('original_url');
    $alias_input = $this->request->getPost('alias_url');   // raw dari user (buat logging)
    $alias_url   = sanitize_alias($alias_input);            // <-- BERSIHKAN DI SINI
    ...
    // Cek keamanan URL (tetap seperti sebelumnya)
    if ($this->check_url_with_virustotal($url)) {
        ...
        $this->sendDiscordNotification($url, $userInfo);
        return $this->response->setJSON(['error' => 'URL Tidak Aman.', 'unsafe' => true]);
    }

    // Validasi alias setelah sanitasi
    if ($alias_input && $alias_url === '') {
        return $this->response->setJSON(['error' => 'Customize URL berisi karakter tidak valid.']);
    }
    ...
    // Base64URL userId (tetap)
    $encoded_userId = rtrim(strtr(base64_encode($userId), '+/', '-_'), '=');

    // Domain HARUS konsisten dengan yang dipakai di redirect()
    $domain = 'https://s.pu.go.id';
    ...
    if (!empty($alias_url)) {
        ...
        $shortened_url = $domain . '/' . $encoded_userId . '/' . $alias_url;
    } else {
        $random_string = $this->random_string(6);
        $shortened_url = $domain . '/' . $encoded_userId . '/' . $random_string;
    }

    // Password & encryption (tetap)
    ...
    // Logging untuk audit sanitasi
    if ($alias_input !== $alias_url) {
        log_message('info', 'Alias sanitized. raw="'.$alias_input.'", clean="'.$alias_url.'"');
    }
...
{
    helper('StringSanitizer');

    $model = new LinkModel();

    // Bersihkan segmen URL dari zero-width & percent-encoding
    $encoded_userId  = canonicalize_segment($encoded_userId);
    $alias_or_random = canonicalize_segment($alias_or_random);
    ...
    // Domain harus sama dengan shorten()
    $domain = 'https://s.pu.go.id';
    $full_shortened_url = $domain . '/' . $encoded_userId . '/' . $alias_or_random;
    ...
    if ($link) {
        ...
        if (!empty($link['expired_at']) && $currentDate > $link['expired_at']) {
            log_message('info', 'Expired link access attempt - URL: ' . $full_shortened_url);
            return view('error/error-410');
        }

        if (!empty($link['is_encrypted']) && !empty($link['password'])) {
    ...
    // Plan B (opsional): coba fallback query tanpa domain (cocokkan per-segmen)
    // Misal jika ada data lama di DB yang beda domain:
    /*
    $query2 = $model->builder();
    $query2->where('deleted_at IS NULL')
           ->where('user_id', $userId)
           ->groupStart()
                ->like('shortened_url', '/' . $encoded_userId . '/' . $alias_or_random, 'before') // .../encoded/alias
                ->orWhere('alias_url', $alias_or_random)
           ->groupEnd()
           ->orderBy('expired_at', 'DESC')
           ->orderBy('created_at', 'DESC');
    $link2 = $query2->get()->getRowArray();
    if ($link2) {
        return redirect()->to($link2['original_url']);
    }
    */

    log_message('warning', 'Failed redirect attempt - URL: ' . $full_shortened_url . ', UserID: ' . $userId);
    return view('error/error-404');
}
